...
if (isset($_POST['dell_user']) and (is_worthy("g") or is_admin())) {
	...
	if($type == 1 || $type == 2) {
		...
		for ($i=0; $i < count($row); $i++) { 
			$dialog_id = $row[$i]['id'];
			$pdo->exec("DELETE FROM messages WHERE dialog_id='$dialog_id'");
		}
...
if (isset($_POST['admin_change_login']) and (is_worthy("f") or is_admin())) {
	...
	if(!$U->check_login_lenght($login)) {
		exit(json_encode(array('status' => '2', 'data' => '<p class="text-danger">Логин должен состоять не более чем из 30</p>')));
	}
...
if (isset($_POST['add_admin']) and (is_worthy("j") or is_admin())) {
	...
	service_log("Управление админами: Выданы права", $admin['id'], $server->id, $pdo);
...
if (isset($_POST['dell_admin_service']) and (is_worthy("j") or is_admin())) {
	...
	$STH = $pdo->prepare("SELECT `services`.`users_group`,`admins__services`.`admin_id`,`servers`.`type` AS `server_type`, `admins`.`server`, `admins`.`user_id` FROM `admins__services` 
		LEFT JOIN `admins` ON `admins`.`id` = `admins__services`.`admin_id` 
		LEFT JOIN `servers` ON `servers`.`id` = `admins`.`server`
		LEFT JOIN `services` ON `services`.`id` = `admins__services`.`service` 
		WHERE `admins__services`.`id`=:id LIMIT 1"); $STH->setFetchMode(PDO::FETCH_OBJ);
	$STH->execute(array( ':id' => $id ));
	$admin = $STH->fetch();
	...
	service_log("Управление админами: Удаление услуги", $admin->admin_id, $admin->server, $pdo);
...
if (isset($_POST['change_admin_days']) and (is_worthy("j") or is_admin())) {
	...
	$STH = $pdo->prepare("SELECT `services`.`name` AS `service_name`, `admins`.`id`, `admins`.`name`, `admins`.`user_id`, `servers`.`type` AS `server_type`, `servers`.`id` AS `server_id`, `servers`.`name` AS `server_name` 
	...
	service_log("Управление админами: Установлен срок окончания прав: ".$date, $admin->id, $admin->server_id, $pdo);
...
if (isset($_POST['change_admin_flags']) and (is_worthy("j") or is_admin())) {
	...
	$STH = $pdo->prepare("SELECT `services`.`rights` AS `service_rights`, `admins`.`id`, `admins`.`name`, `admins`.`user_id`, `servers`.`type` AS `server_type`, `servers`.`id` AS `server_id`, `servers`.`name` AS `server_name` 
			FROM `admins__services` 
		LEFT JOIN `admins` ON `admins__services`.`admin_id` = `admins`.`id` 
		LEFT JOIN `servers` ON `admins`.`server` = `servers`.`id`
		LEFT JOIN `services` ON `admins__services`.`service` = `services`.`id`
	...
	service_log("Управление админами: Смена флагов у прав", $admin->id, $admin->server_id, $pdo);
	exit (json_encode(array('status' => '1')));
}
