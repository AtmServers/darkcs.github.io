...
if (isset($_POST['edit_user_telegram'])) {
	...
	if (empty($user_telegram)) {
		exit('<span class="m-icon icon-remove"></span> Вы не указали логин телеграма!');
	}

	if (mb_strlen($user_telegram, 'UTF-8') < 3 or mb_strlen($user_telegram, 'UTF-8') > 50) {
		exit('<span class="m-icon icon-remove"></span> Логин телеграма должен состоять не менее чем из 3 символов и не более чем из 50.');
	}
	...
	exit('<span class="m-icon icon-ok"></span> Ваш логин телеграма изменен!');
}
if (isset($_POST['edit_first_user_password'])) {
	...
	write_log("Пароль изменен на ".$user_password2); 
...
if (isset($_POST['add_new_friend'])) {
	...
	$row = $STH->fetch();
	if (empty($row->id)){
		exit('Ошибка: [Пользователя не существует]');
	}
	...
	$row = $STH->fetch();
	if (isset($row->id)) {
		exit('Ошибка: [Заявка уже существует]');
	}
	...
	exit('<li><a id="old_button"><span class="m-icon icon-plus"></span> Заявка отправлена</a></li>');
}
if (isset($_POST['cancel_friend'])) {
	...
	$pdo->exec("DELETE FROM users__friends WHERE id_sender='$_SESSION[id]' AND id_taker='$id' LIMIT 1");
	exit('<li><a id="old_button"><span class="m-icon icon-remove"></span> Заявка отменена</a></li>');
}
if (isset($_POST['reject_friend'])) {
	$profile_id = checkJs($_POST['id'],"int");

	if (empty($profile_id)) {
		exit();
	}

	$pdo->exec("DELETE FROM users__friends WHERE id_sender='$profile_id' AND id_taker='$_SESSION[id]' LIMIT 1");

	echo '<li><a id="old_button"><span class="m-icon icon-remove"></span> Заявка удалена</a></li>';
	exit();
}
if (isset($_POST['take_friend'])) {
	...
	$STH = $pdo->query("SELECT id FROM users WHERE id='$profile_id' LIMIT 1"); $STH->setFetchMode(PDO::FETCH_OBJ);
	$row = $STH->fetch();
	if (empty($row->id)){
		exit('Ошибка: [Пользователя не существует]');
	}

	$STH = $pdo->prepare("UPDATE users__friends SET accept=:yes WHERE id_sender='$profile_id' AND id_taker='$_SESSION[id]' LIMIT 1");
	$STH->execute(array(':yes' => "1"));

	echo '<li><a id="old_button"><span class="m-icon icon-ok"></span> Заявка принята</a></li>';
	exit();
}
if (isset($_POST['dell_friend'])) {
	...
	$pdo->exec("DELETE FROM users__friends WHERE (id_sender='$profile_id' AND id_taker='$_SESSION[id]') OR (id_sender='$_SESSION[id]' AND id_taker='$profile_id') LIMIT 1");
	exit('<li><a id="old_button"><span class="m-icon icon-minus"></span> Друг удален</a></li>');
}
...
if (isset($_POST['add_ban'])) {
	...
	if (!empty($bid_db)){
		...
		if ($type == '2' || $type == '3' || $type == '5') {
			...
			$STH = $pdo2->query("SELECT bid FROM $table WHERE server_ip = '$address' and bid='$bid_db' LIMIT 1"); $STH->setFetchMode(PDO::FETCH_OBJ);
		} else {
			...
			$sid = $row->sid;
			$table = set_prefix($db_prefix, 'bans');
			$STH = $pdo2->query("SELECT bid FROM $table WHERE (sid = '$sid' OR sid = '0') and bid='$bid_db' LIMIT 1"); $STH->setFetchMode(PDO::FETCH_OBJ);
		}
		$row = $STH->fetch();
		if (empty($row->bid)){
			exit(json_encode(array('status' => '2')));
